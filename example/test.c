...
int main(int argc, const char *argv[]) {
  ...
  // Some macros are provided which allows explicit value getting in macros.h.
  char **arg_input_val = get_arg_val(args, arg_input_def->name);
  int *arg_somecount_val = (int *)get_arg_val(args, arg_somecount_def->name);
  bool *arg_someflag_val = (bool *)get_arg_val(args, arg_someflag_def->name);
  char **arg_output_val = (char **)get_arg_val(args, arg_output_def->name);
